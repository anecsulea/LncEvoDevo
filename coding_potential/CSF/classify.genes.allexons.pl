...
sub overlapPositiveScores{
	...
	foreach my $strand (keys %{$orderedexons->{$chr}}){
		...
		# else{
		#     print "no positive scores on chromosome ".$chr." strand ".$strand."\n";
		# }
	    }
	    # else{
	    # 	print "no positive scores on chromosome ".$chr.".\n";
	    # }
	}	
    }
}
...
sub readOverlapRepeats{
	...
	if(exists $repeats->{$txid}){
	    push(@{$repeats->{$txid}{"start"}}, $start);
	    push(@{$repeats->{$txid}{"end"}}, $end);
	}
	else{
	    $repeats->{$txid}={"start"=>[$start], "end"=>[$end]};
	}
...
my @defaultpars=("pathAnnotGTF", "pathCoveredRegions", "pathPositiveScores", "minFractionOverlap", "minLengthOverlap", "pathOutputExons", "pathOutputTranscripts", "pathOutputGenes");
