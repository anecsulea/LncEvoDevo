...
do
	...
	do
	    ...
	    if [ -e ${pathResults}/CSF_PositiveScores_windowSize${windowSize}bp_penalty${penalty}_pseudofreq${pseudofreq}_chr${chr}_start${start}_end${end}.txt ]; then
		export paths=${pathResults}/CSF_PositiveScores_windowSize${windowSize}bp_penalty${penalty}_pseudofreq${pseudofreq}_chr${chr}_start${start}_end${end}.txt,${paths}
	    fi
...
perl ${pathScripts}/combine.positive.scores.pl --pathsScores=${paths} --pathOutput=${pathResults}/CSF_PositiveScores_windowSize${windowSize}bp_penalty${penalty}_pseudofreq${pseudofreq}.txt 
