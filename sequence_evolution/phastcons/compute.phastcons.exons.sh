...
do
	...
	if [ ${chr} = Y ]||[ ${chr} = 19 ]||[ ${chr} = 20 ]; then
	    qsub -P P_biometr -q huge -l s_rss=10G,sps=1  -o ${pathScripts}/std_output_phastcons_exons_${species}.txt -e ${pathScripts}/std_error_phastcons_exons_${species}.txt ${pathScripts}/bsub_script	
	else
	    qsub -P P_biometr -q mc_highmem_long -l s_rss=30G,sps=1 -pe multicores 1 -o ${pathScripts}/std_output_phastcons_exons_${species}.txt -e ${pathScripts}/std_error_phastcons_exons_${species}.txt ${pathScripts}/bsub_script	
	fi
