...
sub readCGI{
	...
	my $start=$s[2]+1; ## to make it 1-based 
	my $end=$s[3]+0;
...
$parameters{"pathCpGIslands"}="NA";
$parameters{"maxDistance"}="NA";
$parameters{"pathOutput"}="NA";

my %defaultvalues;
my @defaultpars=("pathGTF", "pathCpGIslands", "maxDistance", "pathOutput");
...
print "Reading and ordering CpG island coordinates...\n";

my %cgi;

readCGI($parameters{"pathCpGIslands"}, \%cgi);

my $nbcgi=keys %cgi;

print "There are ".$nbcgi." CpG islands.\n";

my %orderedcgi;

orderCGICoords(\%cgi, \%orderedcgi);

print "Done.\n";

#####################################################################

print "Extracting overlap between TSS and CpG islands...\n";
...
extractOverlapTSSCGI(\%orderedtss, \%orderedcgi, $margin, \%overlapcgi);
...
print $output "GeneID\tTranscriptID\tChr\tStart\tEnd\tStrand\tTSS\tOverlappingCGI\n";
