...
my %exons;
readGTF($parameters{"pathAnnotGTF"}, \%exons);
...
print $output "TranscriptID\tTotalLength\tCoverageSense\tCoverageAntisense\n";
...
foreach my $txid (keys %txsense){
    ...
    print $output $txid."\t".$totlen1."\t".$covsense."\t".$covantisense."\n";
}
