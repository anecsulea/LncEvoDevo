...
if(load==TRUE){
  ...
  id.orthopc=rownames(tpm.orthopc)
  ...
  ## kmeans for differentially expressed protein-coding genes, both species
  
  kmeans=list()
  
  for(tiss in c("Brain","Kidney", "Liver", "Testis")){
    this.kmeans=read.table(paste(pathDatasets, "SupplementaryDataset4/KmeansClusters_MouseRatOrtho_DiffExp_",tiss,"_ProteinCoding.txt", sep=""), h=T,stringsAsFactors=F)
...
if(prepare==TRUE){
  ...
  for(tiss in tissue.order){
    ...
    signif.both=which(idmouse.orthopc%in%signif.mouse & idrat.orthopc%in%signif.rat)

    this.id.mouse=idmouse.orthopc[signif.both]
    this.id.rat=idrat.orthopc[signif.both]
...
pdf(file=paste(pathFigures, "SupplementaryFigure13.pdf", sep=""), width=6.85, height=10.5)
...
for(tiss in tissue.order){
  ...
  for(age in age.order){
    ...
    mtext(n, at=this.xpos, side=1, line=2.75, cex=0.67, las=2)
  }
  
}

mtext("N =", side=1, line=2.5, at=-1, cex=0.67, las=1)
